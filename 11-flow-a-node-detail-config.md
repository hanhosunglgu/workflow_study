...
>
> ⚠️ **이 2단계 구조도 완전한 근본 해결은 아니다** — Agent #1B의 1차 경량 스캔 자체는 여전히 조직 전체를 페이지네이션으로 훑어야 하므로, 담당자 수 자체가 매우 많은 조직에서는 1차 스캔 단계에서 다시 컨텍스트 예산 문제가 재발할 수 있다. 다만 자산 상세 필드를 버리고 담당자 ID만 누적하므로 페이지당 컨텍스트 사용량은 크게 줄어든다. ~~이 완화책으로도 재발할 경우 방법 3(외부 배치 스크립트로 IVMS API 호출과 페이지네이션 자체를 캔버스 밖에서 처리)을 검토해야 한다.~~ → **4차 실패로 재발이 확인됐고, 방법 3은 아래 제약으로 기각됐다.**

> 🔵 **제약 확정 및 방법 3 기각 (2026-08-21)**
>
> **해결은 ixi-enterprise 캔버스 컴포넌트만으로 이루어져야 한다.** 외부 배치 스크립트·집계 API·MCP 서버 신설은 **범위 밖**이다. 따라서 본 문서가 여러 곳에서 "최후의 수단"으로 지목한 **방법 3(외부 배치 스크립트 분리)은 기각**한다. 이하 본문의 방법 3 관련 서술은 모두 무효이며, 취소선으로 표시했다.
>
> **아울러 담당자 축 설계(#1B 경량 스캔 → #1B-2 `rspnMngId` 정밀조회) 자체가 개발기에서 검증 불가로 확인됐다.** 개발기 자산 1,394건 전 샘플에서 `chrgId` 채움률이 **0%**다(`12-flow-a-restructure-plan.md` 4-1.4절). 2·2-2단계에서 확인된 담당자 2명은 운영기 데이터였다. 따라서 2-7-2절 이하 `rspnMngId` 기반 구성은 **운영기 접근 가능한 망에서만 검증 가능**하다.
>
> **캔버스 한정 제약 하에 남은 유일한 레버는 처리 범위 축소**(유효성이 확인된 `mgmtOrgId` 축)이며, 상세는 `12-flow-a-restructure-plan.md` 7.2절을 따른다. 캔버스로 메울 수 없는 항목(projection 지점 부재·Loop/Function 부재)은 플랫폼 요구사항으로 제기한다(REQ-004 🔴 High 상향).
...
- [ ] 각 Agent 실행 시 `context limit exceeded` 오류가 재발하지 않는지 확인 — 재발 시(특히 Agent #1B의 1차 경량 스캔 단계에서) **처리 범위를 `mgmtOrgId` 하위 조직 단위로 축소**한다(`12-flow-a-restructure-plan.md` 7.2절). ~~방법 3(외부 배치 스크립트 분리)~~은 기각됐다(0절 제약 확정)
...
3. 정상 동작하지 않으면 다음을 순서대로 점검:
   ...
   - **컨텍스트 예산 오류가 여기서도 재발하는지** — 재발하면 담당자 1인당 자산 건수 자체가 상한(600건)보다 많은 조직이라는 뜻이며, **처리 범위를 하위 조직 단위로 축소**해야 한다(`12-flow-a-restructure-plan.md` 7.2절). ~~방법 3(외부 배치 스크립트 분리)~~은 기각됐다(0절 제약 확정)
   - ⚠️ **개발기에서는 이 검증 자체가 불가능하다** — `chrgId` 채움률 0%(0절 제약 확정 참고). 운영기 접근 가능한 망에서 수행할 것
