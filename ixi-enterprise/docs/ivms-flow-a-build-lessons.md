...
> 🔵 **후속 정정 (2026-08-21)** — 위 판단은 당시 기록으로 보존하되, 이후 다음과 같이 뒤집혔다.
>
> **방법 3은 기각됐다.** 해결은 ixi-enterprise 캔버스 컴포넌트만으로 이루어져야 한다는 제약이 확정됐고, 외부 배치 스크립트·집계 API·MCP 서버 신설은 범위 밖이다.
>
> 또한 2026-08-20 curl 실측으로 위 표의 전제 두 가지가 무너졌다(`12-flow-a-restructure-plan.md` 4-1절).
>
> | 당시 판단 | 실측 후 |
> |---|---|
> | 병목은 데이터 양 | **호출 횟수**. 자산을 한 건도 다루지 않는 #1A(`orgList` GET 6회)가 1,017,179 토큰 소진 |
> | 규모 약 1,600건 | **실측 9,794건**(49페이지). 미조치 추정 3,428건 |
> | `#1A 하드코딩 되돌리기` 필요 | **철회.** 재귀 탐색을 런타임에 두는 것이 4차 실패의 직접 원인. 별도 워크플로우로 분리하는 것이 정식 설계 |
> | 담당자 축(`rspnMngId`)으로 스코프 축소 | **개발기 검증 불가.** `chrgId` 채움률 0%(1,394건 전 샘플). 확인됐던 담당자 2명은 운영기 데이터 |
> | 서버 필터로 미조치만 조회 | **무효.** `filter.xorStr` 유무·조건을 바꾼 3회 응답이 MD5까지 동일 |
>
> **현재 채택 방향**: 처리 범위를 `mgmtOrgId` 하위 조직 단위로 축소(12번 문서 7.2절). 캔버스로 메울 수 없는 항목(projection 지점 부재·Loop/Function 부재)은 플랫폼 요구사항으로 제기한다(REQ-004 🔴 High 상향).
